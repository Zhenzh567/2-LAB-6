...
private:
    ...
    void importFromTxt() {
        QString filename = QFileDialog::getOpenFileName(this, "Выберите TXT", "", "Текстовые (*.txt)");
        if (filename.isEmpty()) return;
        QFile file(filename);
...
public:
    MainWindow() {
        ...
        auto* inkLayout = new QHBoxLayout();
        inkLayout->addWidget(new QLabel("Уровень чернил (0-100, можно пусто):"));
        inkEdit = new QLineEdit();
        inkLayout->addWidget(inkEdit);
        formLayout->addLayout(inkLayout);
...
private:
    ...
    void addMarker() {
        int ink = -1;
        QString inkText = inkEdit->text().trimmed();
        if (!inkText.isEmpty()) {
            bool ok;
            ink = inkText.toInt(&ok);
            if (!ok) ink = -1;
        }

        Marker m(nameEdit->text(), descEdit->text(), colorEdit->text(), ink);
        auto markers = Marker::loadAll("markers.json");
        markers.append(m);
        Marker::saveAll("markers.json", markers);
        QMessageBox::information(this, "Готово", "Маркер добавлен");
    ...
    void importFromTxt() {
        ...
        descEdit->setText(m.description);
        colorEdit->setText(m.color);
        inkEdit->setText(m.inkLevel >= 0 ? QString::number(m.inkLevel) : "");
    }
    ...
    void showAll() {
        ...
        for (int i = 0; i < markers.size(); ++i) {
            ...
            table->setItem(i, 1, new QTableWidgetItem(markers[i].description));
            table->setItem(i, 2, new QTableWidgetItem(markers[i].color));
            table->setItem(i, 3, new QTableWidgetItem(markers[i].inkLevel >= 0 ? QString::number(markers[i].inkLevel) : ""));
        }
    }
};
